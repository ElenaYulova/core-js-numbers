...
function roundToPowerOfTen(num, pow) {
  if (Number.isNaN(num) || Number.isNaN(pow)) return 'Wrong param(s)';
 ...
 */
function isPrime(n) {
  if (Number.isNaN(n)) return 'Wrong param(s)';
  if (n === 1) return true;
...
function getCube(num) {
  return Number.isNaN(num) ? 'Wrong param(s)' : num ** 3;
}
...
function getSumToN(n) {
  if (!Number.isFinite(n)) return 'Wrong param(s)';
  return (n * (n + 1)) / 2;
}
...
function getSumOfDigits(num) {
  if (!Number.isFinite(num)) return 'Wrong param(s)';
  const arrNum = num.toString().split('');
  const sum = arrNum.reduce((previousValue, item) => {
...
function getCountOfOddNumbers(number) {
  if (!Number.isFinite(number)) return 'Wrong param(s)';
